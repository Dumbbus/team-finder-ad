...
После этого база данных будет доступна по адресу `127.0.0.1:5436`.
...
Если при `python manage.py migrate` появляется ошибка вида
`UnicodeDecodeError: 'utf-8' codec can't decode byte ...`, обычно это означает,
что Django не смог подключиться к PostgreSQL, а драйвер некорректно показал
русскоязычное сообщение об ошибке.

В проекте используется современный драйвер `psycopg`, поэтому после обновления
зависимостей выполните:

```bash
pip install -r requirements.txt
```

Проверьте, что Docker Desktop запущен, контейнер базы поднят командой
`docker compose up -d`, а в `.env` указаны те же порт и хост, что в
`docker-compose.yml`. Для Windows лучше использовать:
